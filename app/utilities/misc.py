...
def format_uri_links(view, context, model, p):
    """
    ...
    the_link = '<a href="/%s/webrequest?%s">%s</a>' % (admin_url.value, link_args, model.uri)
    return Markup(the_link)


def format_webrequest_links(view, context, model, p):
    """
    Creats a link which searches on the WebRequest FlaskAdmin page for the Uri
    ...
    link_args = urllib.parse.urlencode(args)
    the_link = '<a href="/%s/knownip?%s">%s</a>' % (admin_url.value, link_args, model.ip)
    return Markup(the_link)
